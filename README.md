...
- **LLM providers** — Azure OpenAI or Groq (fast free tier). Switch in Settings.
...
- LLM credentials: Azure OpenAI **or** a free [Groq](https://console.groq.com) API key
- An MCP server endpoint + API key (e.g. ServiceNow via Azure Logic App)
...
```bash
cp backend/.env.example backend/.env   # fill Postgres, LLM, Google OIDC
...
| Azure OpenAI | `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_DEPLOYMENT`, `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET` |
...
| LLM | Azure OpenAI or Groq (OpenAI-compatible) |
